...
void DotSceneLoader::processEntity(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	{
		LogManager::getSingleton().logMessage("[DotSceneLoader] Error loading an entity! " + meshFile + std::string(" in group ")
		+ m_sGroupName + " with name " + name);
...
bool DotSceneLoader::getAttribBool(TiXmlElement *XMLNode, const String &attrib, bool defaultValue)
{
	...
	if(String(XMLNode->Attribute(attrib.c_str())) == String("true"))
		return true;
